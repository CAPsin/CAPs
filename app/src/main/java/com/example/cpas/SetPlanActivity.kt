    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        checkButton.setOnClickListener {
            if(editplan.equals("")) {
                ...
                //val data = planData(editplan.text.toString(), "0")
                val data = mapOf<String, String>(
                    "title" to editplan.text.toString(),
                    "time" to "0"
                )
                
                databaseReference.child("plan").child(dateString).child(editplan.text.toString()).updateChildren(data).addOnSuccessListener {
